...
| **The rest of the dirty-list call sites** | Medium | Slots are wired; a machine breaking, a plant growing and a room type changing still push nothing, so those changes wait for a camera move. Each is one `wh_push` at a site that already knows the dome and the index. |
