...
bds_sz:
        ld      (bd_size),a
        ; Το δωμάτιο οξυγόνου χωράει μόνο σε μικρό θόλο: αλλάζοντας μέγεθος, η
        ; επιλογή μπορεί να έγινε άκυρη μόνη της (§6.5).
        jp      rm_fix
...
bdk_got:
        ...
        inc     hl
        push    hl
        call    rm_room                 ; ό,τι διάλεξε ο παίκτης με πάνω-κάτω
        pop     hl
        ld      (hl),a                  ; δωμάτιο
        inc     hl
        ld      (hl),DS_BUILDING
...
bdk_h:
        ...
        inc     hl
        djnz    bdk_h
        ; ΤΑ ΜΗΧΑΝΗΜΑΤΑ ΜΠΑΙΝΟΥΝ ΤΩΡΑ, ΦΑΙΝΟΝΤΑΙ ΟΤΑΝ ΧΤΙΣΤΕΙ. Κανένα πέρασμα
        ; δεν κοιτάζει θόλο που δεν είναι DS_ACTIVE (§6.5, §6.6) και ο renderer
        ; δείχνει το εργοτάξιο άδειο, οπότε το να γραφτούν στην τοποθέτηση
        ; γλιτώνει ένα δεύτερο πέρασμα στο τέλος του χτισίματος.
        ld      a,(bd_node)
        call    rm_fitout
        jr      bdk_done
bdk_struct:
...
bl_lp:
        ld      a,(bl_i)
        ; ΤΟΝ ΕΑΥΤΟ ΤΟΥ ΔΕΝ ΤΟΝ ΜΕΤΡΑΕΙ. Το εργοτάξιο είναι ήδη «ζωντανό»
        ; (DS_BUILDING) και απέχει μηδέν από τον κέρσορα, άρα ήταν πάντα ο
        ; κοντινότερος: ο έλεγχος «ο εαυτός του» πιο κάτω γύριζε ΧΩΡΙΣ ακμή και
        ; ο θόλος έμενε απρόσιτος για πάντα. Καμία δομή δεν το έδειχνε — μια
        ; δομή παίρνει κόμβο 64+, που δεν ισούται ποτέ με δείκτη θόλου — και
        ; καμία δοκιμή δεν είχε περιμένει θόλο να τελειώσει ως το βήμα 26.
        ld      hl,bd_node
        cp      (hl)
        jr      z,bl_next
        call    ob_dome_live
...
; ΤΟ ΒΙΟΠΛΑΣΤΙΚΟ ΣΦΡΑΓΙΖΕΙ, ΔΕΝ ΣΤΗΡΙΖΕΙ. Οι εξωτερικές δομές — πάνελ,
; ανεμογεννήτρια, συλλέκτης, αντλία, ορυχείο, πίστα — είναι σκελετοί στον αέρα
; και κοστίζουν ΜΕΤΑΛΛΟ μόνο· βιοπλαστικό θέλει ό,τι κρατά ατμόσφαιρα: θόλοι,
; διάδρομοι, αεροθάλαμος. Ως το βήμα 27 τα πάντα ήθελαν και τα δύο, και το
; βιοπλαστικό ήταν το πραγματικό ταβάνι: η αποικία ξόδευε τα τριάντα της στα
; τέσσερα πρώτα κτίσματα και μετά δεν μπορούσε να χτίσει ΤΙΠΟΤΑ, επειδή η
; μόνη πηγή βιοπλαστικού (mach_bioplastic) θέλει θερμοκήπιο και μεσαίο
; εργοστάσιο — δηλαδή δύο θόλους που κοστίζουν βιοπλαστικό.
bld_items:
        ...
        dw t_corr
        db BK_STRUCT,K_SOLAR,   3,3, 15, 0
        dw t_solar
        db BK_STRUCT,K_TURBINE, 3,3, 18, 0
        dw t_turb
        db BK_STRUCT,K_COLLECT, 3,3, 14, 0
        dw t_coll
        db BK_STRUCT,K_EXTRACT, 3,3, 22, 0
        dw t_extr
        db BK_STRUCT,K_MINE,    4,4, 30, 0
        dw t_mine
        db BK_STRUCT,K_AIRLOCK, 2,2, 10, 6
        dw t_lock
        db BK_STRUCT,K_PAD,     4,4, 45, 0
        dw t_pad
...
bd_rsel:    db 0                ; θέση στον κατάλογο δωματίων (src/rooms.asm)
bd_tx:      db 0
bd_ty:      db 0
bd_cy:      db 0
bd_row:     db 0
