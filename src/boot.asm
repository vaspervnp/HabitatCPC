...
;   #3F80         το επίμετρο — ΑΦΟΥ σβήσουν τα ROM
;
...
; στο #3F80 — εκεί φτάνει κανείς μόνο με τα ROM σβηστά, και τότε το AMSDOS δεν
; χρειάζεται πια.
...
; ΤΟ ΕΠΙΜΕΤΡΟ ΕΙΝΑΙ 40 BYTES ΚΑΙ ΚΡΑΤΟΥΣΕ 512. Το #3E00 ήταν στρογγυλός
; αριθμός, και το tools/mkdsk.py τον επιβάλλει ως ταβάνι της τράπεζας 0: ό,τι
; έμενε από κει ως το #4000 ήταν χαμένο, γιατί ο φορτωτής αντιγράφει το
; επίμετρο ΑΦΟΥ φορτώσει το παιχνίδι. Στο #3F80 μένουν 128 bytes για σαράντα —
; και η τράπεζα 0 κερδίζει 384, που ήταν ακριβώς αυτό που έλειπε στο βήμα 26.
; Πιο κάτω δεν πάει: η στοίβα του παιχνιδιού ξεκινά από το #0100 προς τα κάτω
; και το επίμετρο δεν διαβάζει ούτε γράφει τίποτα κάτω από το #4000.
ENDGAME         equ #3F80               ; το επίμετρο, με τα ROM σβηστά
