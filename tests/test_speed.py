...
def main():
    ...
    # επανασχεδίαση HUD κοστίζει 1,15 frames και γίνεται κάθε δεκαέξι, άρα η
    # προσομοίωση παίρνει 16 από τα 17,2. Μετρήθηκε βάζοντας `ret` πάνω από το
    # ml_hud — τότε βγαίνει ακριβώς 1,00. Το όριο εδώ φυλάει το ΚΟΣΤΟΣ ΤΟΥ
    # HUD: αν ανέβει, πέφτει αυτό. Ηταν 0,84 πριν την κρυφή μνήμη του §9.2.
    check(0.88 <= results[1] / N <= 1.00,
          f"το x1 τρέχει στο {results[1] / N:.2f} του ονομαστικού — τα υπόλοιπα "
          f"τα τρώει το HUD (1,15 frames ανά δεκαέξι)")
